...
namespace WindowsFormsApp1
{
    public class DAOCritEval
    {
		public static void SetNote(NpgsqlConnection conn, int offre, string nom_candid, string prenom_candid,int note,string nom_rh, string prenom_rh, string libelle_crit)
        {
            ...
            string sql1 = "SELECT COUNT(id_evaluation) FROM EVALUATION WHERE EVALUATION.nom_rh_evaluation = '" + nom_rh + "' AND EVALUATION.prenom_rh_evaluation = '" + prenom_rh + "' AND id_candidature = (SELECT id_candidature FROM CANDIDATURE WHERE id_offre_emplois = " + offre + " AND nom_candidature = '" + nom_candid + "' AND prenom_candidature = '" + prenom_candid + "');";
            ...
            string sql3 = "SELECT id_evaluation FROM EVALUATION WHERE EVALUATION.nom_rh_evaluation = '" + nom_rh + "' AND EVALUATION.prenom_rh_evaluation = '" + prenom_rh + "' AND id_candidature = (SELECT id_candidature FROM CANDIDATURE WHERE id_offre_emplois = " + offre + " AND nom_candidature = '" + nom_candid + "' AND prenom_candidature = '" + prenom_candid + "');";
            ...
            int verifnote = -1;
            string sql4 = "SELECT COUNT(NOTER.note) FROM NOTER WHERE NOTER.id_evaluation = " + eval + " AND id_critere = (SELECT CRITERE.id_critere FROM CRITERE WHERE CRITERE.libelle_critere = '" + libelle_crit + "');";
            ...
            if (verifnote == 1)
            {
                ...
                string sql5 = "UPDATE NOTER SET note = " + note + " WHERE id_evaluation = (SELECT EVALUATION.id_evaluation FROM EVALUATION JOIN CANDIDATURE ON CANDIDATURE.id_candidature = EVALUATION.id_evaluation WHERE CANDIDATURE.nom_candidature = '" + nom_candid + "' AND CANDIDATURE.prenom_candidature = '" + prenom_candid + "' AND CANDIDATURE.id_offre_emplois = " + offre + " AND EVALUATION.nom_rh_evaluation = '" + nom_rh + "' AND EVALUATION.prenom_rh_evaluation = '" + prenom_rh + "') AND id_critere = (SELECT CRITERE.id_critere FROM CRITERE INNER JOIN ASSOCIER ON ASSOCIER.id_critere = CRITERE.id_critere WHERE ASSOCIER.id_offre_emplois = " + offre + " AND CRITERE.libelle_critere = '" + libelle_crit + "'  );";
            ...
            else
            {
                ...
                string sql6 = "INSERT INTO NOTER(note, id_evaluation, id_critere) VALUES(" + note + ", " + eval + ",(SELECT CRITERE.id_critere FROM CRITERE INNER JOIN ASSOCIER ON ASSOCIER.id_critere = CRITERE.id_critere WHERE ASSOCIER.id_offre_emplois = 1 AND CRITERE.libelle_critere = '" + libelle_crit + "'  ));";
